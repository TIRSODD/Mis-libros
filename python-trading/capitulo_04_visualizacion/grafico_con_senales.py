# Capítulo 4 - Marcando señales de compra y venta sobre el gráfico
...
# Generamos señales: 1 = Comprar, -1 = Vender, 0 = Mantener
df['Senal'] = 0
df.loc[df['Close'] > df['SMA_3'], 'Senal'] = 1
df.loc[df['Close'] < df['SMA_3'], 'Senal'] = -1

# Creamos las columnas para los marcadores de compra y venta
df['Compra'] = np.where(df['Senal'] == 1, df['Low'] * 0.99, np.nan)
df['Venta'] = np.where(df['Senal'] == -1, df['High'] * 1.01, np.nan)

# Preparamos los marcadores
apds = [
    mpf.make_addplot(df['SMA_3'], color='blue', width=1.5),
    mpf.make_addplot(df['Compra'], type='scatter', markersize=100, marker='^', color='green'),
    mpf.make_addplot(df['Venta'], type='scatter', markersize=100, marker='v', color='red')
]

# Graficamos
mpf.plot(df, type='candle', style='charles', title='Señales de Trading',
         ylabel='Precio ($)', volume=True, addplot=apds)
